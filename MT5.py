...
class MT5:
    ...
    def orders(symbol, lot, buy=True, id_position=None):
        ...
        # print("HASAN  " , symbol , "    ", mt5.symbol_info(symbol))
        # Get filling mode
        filling_mode = mt5.symbol_info(symbol).filling_mode - 1

        # print("HASAN  " , symbol , "    ", mt5.symbol_info_tick(symbol))
        # Take ask price
        ...
        for i in range(len(result)):
            ...
            if row["position"][0] == 0:
                res = MT5.orders(row["symbol"][0], row["volume"]
                                 [0], buy=True, id_position=row["ticket"][0])
